...
const priorityInd = document.querySelector(
...
const overdueInd = document.querySelector(
...
function updateTime() {
  ...
  if (diff <= 0) {
    ...
    if (days > 0) label = `Overdue by ${days} day${days !== 1 ? "s" : ""}`;
      ...
      label = `Overdue by ${hours} hour${hours !== 1 ? "s" : ""}`;
    ...
    if (days > 1) label = `Due in ${days} days`;
      ...
      label = `Due in ${hours} hour${hours !== 1 ? "s" : ""} ${remMinutes} minute${remMinutes !== 1 ? "s" : ""}`;
    else if (hours > 0) label = `Due in ${hours} hour${hours !== 1 ? "s" : ""}`;
    else if (totalMinutes > 0)
...
function applyState() {
  ...
  card.classList.toggle("in-progress", state.status === "In Progress" && !state.done);
...
function updateStatusSelectStyle() {
  ...
  if (state.status === "In Progress")
    statusControl.classList.add("status-inprogress");
}
...
editForm.addEventListener("keydown", (e) => {
  ...
  const focusable = Array.from(
    editForm.querySelectorAll("input, textarea, select, button:not([hidden])"),
